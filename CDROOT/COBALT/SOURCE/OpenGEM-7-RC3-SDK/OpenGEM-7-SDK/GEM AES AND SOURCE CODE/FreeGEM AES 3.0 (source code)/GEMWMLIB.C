...
WORD w_bldactive(REG WORD w_handle)
{
	...
	pw = &D.w_win[w_handle];
						/* set if it is on top	*/
	istop = (gl_wtop == w_handle || pw->w_cotop == gl_wtop);
	...
	if ( havevbar )
	{
	  ...
	  if (gl_opts.frame3d) 
	       w_bldbar(kind, istop || issub, W_VBAR, pw->w_vslide, 
			pw->w_vslsiz, t.g_x + 2, 0, 
			t.g_w + 4, t.g_h + 4);

	  else w_bldbar(kind, istop || issub, W_VBAR, pw->w_vslide, 
			pw->w_vslsiz, t.g_x, 0, 
			t.g_w + 2, t.g_h + 2);
	}
						/* do horiz area	*/
	if ( havehbar )
	{
	  ...
	  if (gl_opts.frame3d)
	       w_bldbar(kind, istop || issub, W_HBAR, pw->w_hslide,
			pw->w_hslsiz, 0, t.g_y + 2, 
			t.g_w + 4, t.g_h);
	  else w_bldbar(kind, istop || issub, W_HBAR, pw->w_hslide,
			pw->w_hslsiz, 0, t.g_y, 
			t.g_w + 2, t.g_h + 2);
	}
						/* do sizer area	*/
...
WORD draw_change(REG WORD w_handle, REG GRECT *pt)
{
...
#if MULTIAPP
	{
	  ...
	  else w_newmenu(D.w_win[gl_wtop].w_owner);
	}
#endif
						/* set ctrl rect and	*/
						/*   mouse owner	*/
	  ...
	  if ( (pt->g_w == c.g_w) && (pt->g_h == c.g_h) )
	  {
	    ...
	    if ( (w_handle != W_TREE[ROOT].ob_tail) ||
		 (w_handle == oldtop) )
	      ...
	      return(TRUE);
						...
						/*   change		*/
	    diffbord = D.w_win[oldtop].w_cotop==gl_wtop ? FALSE : 
			  !( (D.w_win[oldtop].w_flags & VF_SUBWIN) &&
		          (D.w_win[gl_wtop].w_flags & VF_SUBWIN) );
						/* draw oldtop covered	*/
						/*   with deactivated 	*/
						/*   borders		*/
	    if (oldtop != NIL)
	    {
	      if (diffbord)
	      {
		  	...
		  	if (D.w_win[oldtop].w_cotop != NIL)
		  	{
		      w_clipdraw(D.w_win[oldtop].w_cotop, 0, MAX_DEPTH, 2);
	        }
	      }
	...
	REG WORD	*poutwds;
{
	while (po)
	{
	  rc_copy((GRECT *)&po->o_x, (GRECT *)&poutwds[0]);
...
wm_start()
{
	...
	*((BYTE FAR *)(&(tree+ROOT)->ob_spec)) = CC_DESKTOP ;
	W_TREE[ROOT].ob_flags = USECOLORCAT;
	/* 910327WHF: end */
#endif
	...
	{
		W_TREE[ROOT].ob_type = tree[ROOT].ob_type = G_DTMFDB;
		W_TREE[ROOT].ob_spec = tree[ROOT].ob_spec = DTMFDB_EMS_SPEC; 
	}
#endif
...
//	W_ACTIVE[ROOT].ob_state = SHADOWED;
	...
	w_setsize(WS_WORK, DESKWH, &gl_rfull);
						/* init global vars	*/
	gl_wtop = NIL;
	gl_wtree = (LPTREE)ADDR(&W_TREE[ROOT]);
	...
	WORD		w_handle;
{
	...
	D.w_win[w_handle].w_owner = (PD *)NULLPTR;
#if MULTIAPP
 ...
 */
	...
	liketop = ( ( w_handle == gl_wtop ) || ( pwin->w_cotop == gl_wtop) ||
	...
	switch(w_field)
	{
	  ...
	  case WF_CXYWH:
		...
		break;
	  case WF_COTOP:
		D.w_win[gl_wtop].w_cotop = w_handle;
		if (w_handle != NIL)
		{
				/* Fall thru and make this the new top */
			D.w_win[w_handle].w_cotop = gl_wtop;
		}
		else
			break;
	  case WF_TOP:
