...
MLOCAL WORD find_obj    (LPTREE tree, WORD start_obj, WORD which, WORD extype);
MLOCAL WORD fm_inifld   (LPTREE tree, WORD start_fld);
...
MLOCAL WORD find_obj(LPTREE tree, WORD start_obj, WORD which, WORD extype)
{
   ...
   while ( obj >= 0 )
   {
      LPOBJ addr = tree + obj * sizeof(OBJECT);
      theflag = LWGET(addr + 8);
      if (flag)
      {
