...
#include "djgppgem.h" /* All GEM includes */
...
VOID eraser(WORD x, WORD y);			/* erase rectangle of eraser size at x,y*/
...
VOID move_do(LPTREE tree, WORD obj, WORD inc);
...
int 	file_handle;			/* file handle -> pict ld/sv	*/
BYTE	file_name[PATH_MAX] = "";	/* current pict file name	*/
...
DJUSERBLK brushab[6]; 			/* 6 Programmer defined objects	*/
X_BUF_V2  gl_xbuf;


LPLONG	low_color_sel;
...
LPWORD low_erase_fine, low_erase_medium, low_erase_broad;
...
VOID rast_op(WORD mode, GRECT *s_area, MFDB *s_mfdb, 
                        GRECT *d_area, MFDB *d_mfdb)	/* bit block level trns	*/
{
	WORD	pxy[8];
...
/*	vdi_fix			*/
/*------------------------------*/
VOID vdi_fix(MFDB *pfd, LPVOID theaddr, WORD wb, WORD h)  	/* set up MFDB for transform		*/
{
	pfd->fww = wb >> 1;     	/* # of bytes to words 		*/
	pfd->fwp = wb << 3;  		/* # of bytes to to pixels 	*/
	pfd->fh = h;			/* height in scan lines		*/
	pfd->np = 1; 			/* number of planes		*/
	pfd->mp = theaddr;		/* (linear) memory pointer	*/
}

/*------------------------------*/
/*	vdi_trans		*/
/*------------------------------*/
VOID vdi_trans(LPBYTE saddr, WORD swb, LPBYTE daddr, WORD dwb, WORD h) 	/* 'on the fly' transform 	*/
{
...
/*	set_select		*/
/*------------------------------*/
VOID set_select(LPTREE tree, WORD obj, WORD init_no, LPLPTR bind, LPLONG arry)
/* initialize the color	selector objects in Pen/Eraser Dialog  	*/ 
{						
	WORD	n, nobj, cobj, count; 

	indir_obj(tree, obj); 			
	LSSET(bind, LSGET(OB_SPEC(obj))); 		
	LSSET(OB_SPEC(obj), bind);	
	LSSET(bind + 4, arry);			

	n = LWGET(arry);	/* Assumes little-endian! */

	count = 0;
	for (cobj = LWGET(OB_HEAD(obj)); cobj != obj; cobj = LWGET(OB_NEXT(cobj)))		
	{ 	/* loop to init color selector with colors 1 to 4 	*/

		indir_obj(tree, cobj);
		LSSET(OB_SPEC(cobj),  arry + 4*(count + 1) );
		count = (count + 1) % n;
	}

	nobj = LWGET(OB_NEXT(obj)); 	/* set pointer to current color	*/
	indir_obj(tree, nobj);
	LSSET(OB_SPEC(nobj), arry + 4 * (1 + (init_no % n)) );
}
...
WORD	obj;			/* Dialog interaction			*/
{
	...
	bind = LSGET(OB_SPEC(obj));
	dir_obj(tree, obj);
	LSSET(OB_SPEC(obj), LSGET(bind));
	arry = LSGET(bind + sizeof(LONG) );

	for (cobj = LWGET(OB_HEAD(obj)); cobj != obj;
	cobj = LWGET(OB_NEXT(cobj)))
	{
		dir_obj(tree, cobj);
		LSSET(OB_SPEC(cobj), LSGET(LSGET(OB_SPEC(cobj))));
	}

	nobj = LWGET(OB_NEXT(obj));
	dir_obj(tree, nobj);
	temp = LSGET(OB_SPEC(nobj));
	LSSET(OB_SPEC(nobj), LSGET(temp));
...
/*------------------------------*/
VOID move_do(LPTREE tree, WORD obj, WORD inc) 	
				/* routine to scroll the color selector	*/
				/*  in the Pen/Eraser Dialog		*/
{
	WORD	cobj; 
...
{  					
	...
	switch (xtype) {
		case X_SEL: 			/* selected color */
			obspec = LSGET(OB_SPEC(obj));
			obj = get_parent(tree, obj);
			obj = LWGET(OB_NEXT(obj));
			LSSET(OB_SPEC(obj), obspec);
			redraw_do(tree, obj);
			break;
...
/*	dr_code			*/
/*------------------------------*/   
WORD dr_code(PARMBLK *pb)		/* called by FARDRAW.ASM when 	*/
...
{					/* objects in Pen/Eraser Dialog */
	...
	userbrush_mfdb.mp = LSGET(BI_PDATA(taddr)); /* point to data	*/
...
WORD	which;
{
	ob_dostate(tree, which, SELECTED);
}
...
WORD	which;
{
	ob_undostate(tree, which, SELECTED);
}
...
VOID dir_obj(LPTREE tree, WORD which)
{
	ob_undoflag(tree, which, INDIRECT);
}
...
/*	dial_name		*/
/*------------------------------*/
WORD dial_name ( LPBYTE name )		/* dialogue box input filename		*/
{		    
	LONG	tree ;
	LONG	ted_addr ;
	BYTE	c ;	
	...
	ted_addr = LSGET(OB_SPEC(DEMONAME));	/* get obspec pointer 	*/
	LSSET( ted_addr,  name ) ;		/* set obspec pointer 	*/
	LWSET( TE_TXTLEN(ted_addr),9); 		/* 1 more than 		*/
	LBSET( name, '\0');	/* null to clear edit field and position cursor	*/
		...
		== DEMOSOK)
	{
		...
		if ( LBGET(name) )
			dosmemput(".DOO", 5, name + j); /* strcpy( &name[j], ".DOO" ); // add extension	*/
...
BOOLEAN	loop;
{
	...
	while (TRUE)
	{
		...
		if (gl_xbuf.arch)
		{
			fsel_exinput(file_name, fs_iinsel, &fs_iexbutton, " Choose file to load ");
		}
		else
		{
		...
		if (fs_iexbutton)
		{
			...
			DOS_ERR = (_dos_open(file_name, O_RDWR, &file_handle));
...
WORD 	title, item;
{
	...
	case DEMOFILE: /* File menu */
		...
		case DEMOABAN: /* Abandon File */
			_dos_open(file_name, O_RDWR, &file_handle);
			do_load(FALSE);
			break;
...
/*	do_about		*/
/*------------------------------*/
VOID do_about(VOID)			/* display Demo Info... 		*/
{
	LONG	tree, ted_addr;
	GRECT	box;
	...
	/* Set the "AES ID" string */
	ted_addr = LSGET(OB_SPEC(AESID));	/* get obspec pointer 	*/

	LSSET( ted_addr,  gl_xbuf.info ) ;	/* set text pointer 	*/
	LWSET( TE_TXTLEN(ted_addr), LSTRLEN(gl_xbuf.info)+1);
...
/*	do_load			*/
/*------------------------------*/
VOID do_load(BOOLEAN need_name)		/* load demo picture file		*/
{
...
/*	do_save			*/
/*------------------------------*/
WORD do_save(VOID)		/* save current named demo picture	*/
{
	if (*file_name)
	{
		DOS_ERR = _dos_open(file_name, O_RDWR, &file_handle);

		if (!DOS_ERR)	/* File already exists 	*/
		{
		...
		if(DOS_ERR) DOS_ERR = _dos_creat(file_name, _A_NORMAL, &file_handle); 
		if(DOS_ERR)
...
/*	do_save_as		*/
/*------------------------------*/
VOID do_svas(VOID)			/* save demo picture as named		*/
{
	dosmemput(name, 1 + strlen(name), __tb);

	if (dial_name(__tb))
	{ 
		dosmemget(__tb, 1 + LSTRLEN(__tb), name );
		if (name[0] != '\0')
		{
...
/*	set_eraser		*/
/*------------------------------*/
VOID set_eraser(WORD pen, WORD height, LPBYTE eraser) /* set mouse form to eraser		*/
{
	demo_pen = pen;
	demo_height = height;
...
/*	set_color		*/
/*------------------------------*/
VOID set_color(LPTREE tree, WORD obj, WORD color_num, LPLPTR bind)	/*  Set Pen Color Selection	*/
{
	set_select(tree, obj, color_num - 1, bind, low_color_sel);
}
...
/*	do_penselect		*/
/*------------------------------*/
VOID do_penselect(VOID)		/* use dialogue box to input selection	*/
{				/*   of specified pen/eraser		*/
	...
	bind = dos_alloc(8);	/* [JCE] moved to low memory, GEM */
				/* needs to see it */
...
/*	do_erase		*/
/*------------------------------*/
VOID do_erase(VOID)		/* clear the screen and the undo buffer	*/
{ 
...
/*	do_redraw		*/
/*------------------------------*/
VOID do_redraw(WORD wh, GRECT *area)		/* redraw specified area from undo bfr	*/
{
	GRECT	box;
...
/*	do_full			*/
/*------------------------------*/
VOID do_full(WORD wh)	/* depending on current window state, either make window*/
...
/*	demo_term		*/
/*------------------------------*/
VOID demo_term(WORD term_type)
{
	switch (term_type)	/* NOTE: all cases fall through		*/
	{
		case (0 /* normal termination */):

/* [JCE] Save some settings as properties, if it's possible */
...
{					/*  set up Programmer Defined 	*/
	...
	dj_userdef_prealloc(6);	/* 6 USERBLKs */
	...
	/* [JCE] Use dj_userdef to handle 16bit -> 32bit callbacks */

		brushab[nobj].ub_code = dr_code;	
		brushab[nobj].ub_parm = LSGET(OB_SPEC(tr_obj));
		dj_userdef(tree, tr_obj, &brushab[nobj]);
	}
}
...
WORD demo_init()
{
	...
	/* If no info string was returned by GEM, set a default one. */
	if (!gl_xbuf.arch) 
	{
		gl_xbuf.info = dj_string_addr("Digital Research GEM");
	}
	...
	{    	
		...
		graf_mouse(ARROW, 0x0L);
		dj_form_alert(1,
		"[3][Fatal Error !|DEMO.RSC|File Not Found][ Abort ]");
		return(1);
	}
	...
	/* [JCE] Note: calls to dos_alloc() amalgamated to save time and
                memory */
	buff_location  = dos_alloc(buff_size + 222 + 4*(N_COLORS + 1));
	low_color_sel  = buff_location + buff_size;
	low_erase_fine = buff_location + buff_size + 4*(N_COLORS + 1);
	undo_mfdb.mp   = buff_location;

	if (undo_mfdb.mp == 0)
	{
		dj_form_alert(1, "[3][Out of memory][ Abort ]");
	...
	{ 
 	...
 	dj_wind_setl(demo_whndl, WF_NAME, dj_string_addr(wdw_title), 0); 
	...
	/* [JCE] Copy various things from this program into user memory */
	dosmemput(color_sel, 4*(N_COLORS + 1), low_color_sel);

	low_erase_medium = low_erase_fine + 74;
	low_erase_broad  = low_erase_fine + 148;

	dosmemput(erase_fine,   74, low_erase_fine);
	dosmemput(erase_medium, 74, low_erase_medium);
	dosmemput(erase_broad,  74, low_erase_broad);
